...
namespace SlimeVRInstaller.Installer
{
    ...
    public static class RegistryUtils
    {
        ...
        private static string? GetWebViewVersion(RegistryKey? key)
        {
            ...
            if (
                regVal != null
                && regVal is string regValString
                && !string.IsNullOrWhiteSpace(regValString)
                && regValString != "0.0.0.0"
            )
            {
                return regValString;
            }
        ...
        public static bool IsSlimeVRInstalled =>
            Registry.LocalMachine.OpenSubKey(
                "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\SlimeVR"
            ) != null;

        public static void RegisterInstallation(string installPath)
        {
            var regKey = Registry.LocalMachine.CreateSubKey(
                "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\SlimeVR"
            );
            ...
            regKey.SetValue(
                "URLUpdateInfo",
                "https://github.com/SlimeVR/SlimeVR-Installer/releases"
            );
        }
